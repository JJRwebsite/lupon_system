...
exports.getAllChairpersons = async (req, res) => {
  try {
    const db = await connectDB();
    const [rows] = await db.query('SELECT id, firstname, lastname, middlename, status, date_added FROM lupon_chairperson');
    ...
    console.error('Failed to fetch chairpersons:', err);
    res.status(500).json({ error: 'Failed to fetch chairpersons' });
  }
};

exports.addChairperson = async (req, res) => {
  ...
  try {
    const db = await connectDB();
    await db.query(
      'INSERT INTO lupon_chairperson (firstname, lastname, middlename, status, date_added) VALUES (?, ?, ?, ?, ?)',
      [first.toUpperCase(), last.toUpperCase(), middle.toUpperCase(), status || 'Active', dateAdded || new Date()]
    );
    res.status(201).json({ message: 'Chairperson added' });
    await db.end();
  } catch (err) {
    console.error('Failed to add chairperson:', err);
    res.status(500).json({ error: 'Failed to add chairperson' });
  }
};

// Edit a chairperson
exports.editChairperson = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const db = await connectDB();
    await db.query(
      'UPDATE lupon_chairperson SET firstname = ?, lastname = ?, middlename = ? WHERE id = ?',
      [first.toUpperCase(), last.toUpperCase(), middle.toUpperCase(), id]
    );
    res.json({ message: 'Chairperson updated' });
    await db.end();
  } catch (err) {
    console.error('Failed to update chairperson:', err);
    res.status(500).json({ error: 'Failed to update chairperson' });
  }
};

// Remove a chairperson
exports.removeChairperson = async (req, res) => {
  const { id } = req.params;
  try {
    const db = await connectDB();
    await db.query('DELETE FROM lupon_chairperson WHERE id = ?', [id]);
    res.json({ message: 'Chairperson removed' });
    await db.end();
  } catch (err) {
    console.error('Failed to remove chairperson:', err);
    res.status(500).json({ error: 'Failed to remove chairperson' });
  }
}; 