...
function StartMediationModal({ open, onClose, mediation, onSave, onReschedule, setShowSettlementSuccess }: { open: boolean, onClose: () => void, mediation: any, onSave: (minutes: string, images: File[]) => void, onReschedule: (date: string, time: string, rescheduleReason?: string) => void, setShowSettlementSuccess: (show: boolean) => void }) {
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      const isSelected = reschedDate === currentDate.toISOString().split('T')[0];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (processType === "start") {
      ...
      if (bookedTimes.includes(reschedTime)) {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden border border-blue-200">
        <div className="flex justify-between items-center px-8 py-6 bg-gradient-to-r from-green-700 to-green-800 text-white">
          <div className="flex items-center gap-3">
            <PlayCircleIcon className="h-8 w-8 text-green-100" />
            <div>
              <h2 className="text-2xl font-bold capitalize">{processType === "start" ? "Start Mediation" : processType === "reschedule" ? "Reschedule Mediation" : "Settle Mediation"}</h2>
              <p className="text-green-100 text-sm">Mediation ID #{mediation.id}</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-120px)]">
          <form className="p-8 space-y-8" onSubmit={handleSubmit}>
            <div>
              <label className="block font-semibold text-green-700 mb-2">Type of Process</label>
            ...
            {processType === "start" && (
              <>
                <div>
                  <label className="block font-semibold text-green-700 mb-2">Mediation Minutes</label>
                ...
                <div>
                  <label className="block font-semibold text-green-700 mb-2">Mediation Documentation (Upload Photos)</label>
            ...
            {processType === "reschedule" && (
              <div className="space-y-6">
                <div>
                  <label className="block font-semibold text-green-700 mb-3">New Schedule Date</label>
                            ...
                            className={`
                              ...
                              ${
                                day.isSelected
                                  ...
                                  : day.isAvailable
                                  ? 'hover:bg-green-100 text-gray-700 font-medium'
                                  : day.isCurrentMonth
                                  ? 'text-gray-400 cursor-not-allowed'
                                  : 'text-gray-300 cursor-not-allowed'
                              }
                              ${day.isToday && !day.isSelected ? 'ring-2 ring-green-400' : ''}
                            `}
                          >
                            {day.day}
                          </button>
                  ...
                  <div className="space-y-3">
                    {loadingSlots ? (
                      ...
                      <div className={`p-4 rounded-lg border ${
                        slotInfo.isFull ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'
                      }`}>
                        <div className="flex items-center gap-2 mb-2">
                          <div className={`w-3 h-3 rounded-full ${
                            slotInfo.isFull ? 'bg-red-500' : 'bg-green-500'
                          }`}></div>
                          <span className={`font-semibold ${
                            slotInfo.isFull ? 'text-red-700' : 'text-green-700'
                          }`}>
                ...
                <div>
                  <label className="block font-semibold text-green-700 mb-3">New Schedule Time</label>
                    ...
                    <div className="p-3">
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        {[
                          ...
                          const isDisabled = noDateSelected || isBooked || (slotInfo?.isFull && !isSelected);
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                // Prevent any action if no date is selected
                                if (noDateSelected) {
                                  return;
                                }
                                if (!isDisabled) {
                ...
                <div>
                  <label className="block font-semibold text-green-700 mb-2">Reason for Reschedule</label>
            ...
            {processType === "settled" && (
              <>
                <div>
                  <label className="block font-semibold text-green-700 mb-2">Agreements</label>
                  ...
                  <button type="button" className="border px-3 py-1 rounded text-green-700 bg-white hover:bg-green-50 mt-2" onClick={handleAddAgreement}>+ Add Agreement</button>
                </div>
                <div className="mt-4">
                  <label className="block font-semibold text-green-700 mb-2">Remarks</label>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" className="px-4 py-2 rounded bg-green-700 text-white font-semibold">Save</button>
            </div>
          </form>
        </div>
      </div>
...
function SetConciliationModal({ open, onClose, onSave, luponMembers }: { open: boolean, onClose: () => void, onSave: (date: string, time: string, members: string[]) => void, luponMembers: { id: number; name: string }[] }) {
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      const isPast = currentDate < today;
      const isWeekend = currentDate.getDay() === 0 || currentDate.getDay() === 6;
      const isSelected = date === currentDate.toISOString().split('T')[0];
      const isAvailable = isCurrentMonth && !isPast && !isWeekend;
  ...
  const fetchSlotAvailability = async (selectedDate: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Backend returns data nested in data property for mediation endpoint
        setSlotInfo({
          availableSlots: data.data.availableSlots,
          usedSlots: data.data.usedSlots,
          maxSlots: data.data.maxSlotsPerDay,
...
function SetArbitrationModal({ open, onClose, onSave, arbitrators, selectedCase }: { open: boolean, onClose: () => void, onSave: (date: string, time: string, members: string[]) => void, arbitrators: { id: string; name: string }[], selectedCase?: any }) {
  ...
  return !open ? null : (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" aria-hidden="true" />
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden border border-blue-200">
        <div className="flex justify-between items-center px-8 py-6 bg-gradient-to-r from-amber-700 to-amber-800 text-white rounded-t-3xl">
          <div className="flex items-center gap-3">
            <ScaleIcon className="h-8 w-8 text-amber-100" />
            <div>
              <h2 className="text-2xl font-bold">Set Arbitration</h2>
              <p className="text-amber-100 text-sm">Schedule arbitration session with slot validation</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-amber-500/20 rounded-full transition-all duration-200 group">
            <XMarkIcon className="h-6 w-6 text-white group-hover:scale-110 transition-transform" />
          </button>
        </div>
        <div className="overflow-y-auto max-h-[calc(90vh-120px)]">
...
export default function MediationPage() {
  ...
  const filteredCases = latestMediationCases.filter((c) =>
    c.case_title.toLowerCase().includes(searchQuery.toLowerCase()) ||
    c.complainants.some((com: string) => com.toLowerCase().includes(searchQuery.toLowerCase())) ||
    c.respondents.some((res: string) => res.toLowerCase().includes(searchQuery.toLowerCase()))
  );
        ...
        <div className="bg-white rounded-xl shadow p-0">
          <div className="flex items-center justify-between bg-blue-400 rounded-t-xl px-6 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button
                className="bg-white border border-blue-500 text-black-300 px-3 py-1 rounded font-semibold hover:bg-blue-50"
                  ...
                  return (
                    <tr key={c.id} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      ...
                      <td className="px-4 py-2">
                        <button className="border px-3 py-1 rounded bg-gray-50 hover:bg-gray-100">Forms</button>
                      </td>
                    </tr>
                  );
                })}
